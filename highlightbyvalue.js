            ...
            <!-- /*
            need a way to skip the header (maybe just telling user to use thead and th)
            BLEND: from one olor to another
            SORT by date, time, text, value...
        
            can MAP() function be used to traverse the table?
            use closures instead of creating global vars (I don't like how arrayHeight and var k are scoped). Each section should use a closure so all those variables don't float around
            ...
            TODO: 
                  : Option to try dataAttribute first and then check <td> (fallbackToCell:T/F) ?
                  : automatically calculate colorGradeMaxLight
                  : ignore zero values (false by default)
                  : tell the function you want to blend the color with the color that is already there
            REFACTORING
                : pull out cellValue parsing
                : only reference non-default values (this way anything passed in incorrectly will just revert to default)
            */-->
